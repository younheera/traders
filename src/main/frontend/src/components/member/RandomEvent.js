 ...
 * @modify date 2023-10-25 14:26:07
...
const RandomEvent = () => {
  ...
  let maxY = document.body.clientHeight - 300; //웹 브라우저 상단 네비바 고려

  const moveButton = () => {
    const randomX = Math.random() * maxX;
    const randomY = 200 + Math.random() * (maxY - 200);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const newMaxY = 200 + Math.random() * (maxY - 200);
