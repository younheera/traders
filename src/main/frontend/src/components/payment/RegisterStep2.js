 ...
 * @modify date 2023-09-27 14:23:05
 * 그린페이 가입 - 2. 문자인증 처리
 */
...
const RegisterStep2 = ({inputAuthNum,authNum,onText,onNext}) => {
    ...
    return (
        <div>
            <h2>문자 인증</h2>
            <button name='authNumBtn' onClick={toggleAuthNumBtn} disabled={authBtnFlag}>인증번호 받기</button><br/>
            {authBtnFlag && 
                <div>
                    <input type='text' name='inputAuthNum' maxLength={6} size={6} onChange={onText}/>
                    <button onClick={()=>authorizeNum(inputAuthNum,authNum)}>인증</button>
                </div>
            }
        </div>
    );
